...
define(['angular','css!./customer.css','common/script/lib/swiper.min.js'], function(angular) {
	...
	/*申请售后*/
	ApplyController.$inject = ['$scope', '$state','$verifyService','$timeout','$log','$customerService','$stateParams','$window','$productService'];
	function ApplyController($scope, $state,$verifyService,$timeout,$log,$customerService,$stateParams,$window,$productService){
    		...
    		if(vm.param.examreport != "1" && vm.param.examreport != "2"){
				...
				$customerService.saveAfterSaleApply({
					...
					if(data.resultCode == "0000"){
						...
						$state.go('return',{
							orderId : $stateParams.orderId,
    						afterSaleApplyId : vm.afterSaleApplyId,
    		...
    		$customerService.getAfterSaleApplyInfo({
    			...
    			if(data.resultCode=="0000"){
    				if(isEmptyObject(data.result)){
    					...
    					vm.stages = data.result.result.afterSaleApply.serviceType;
    	...
    	function imges(){
    		...
    		$customerService.doGoodsDetail({
    			...
    			if(data.result.typeFrom == 2){
	    			...
	    			vm.imageUrl = $productService.imgUrl[4]+data.result.imageUrl;
	    			vm.Price = data.result.price;
    				...
    				vm.goodsName = data.result.goodsName;
	    			vm.goodsNub = data.result.goodsNub;
	    			vm.imageUrl = imgUrl+data.result.imageUrl;
	    			vm.Price = data.result.price;
	    			vm.num = data.result.goodsNum;
    			}
