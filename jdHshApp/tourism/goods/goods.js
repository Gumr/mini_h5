	...
	function goodsCtrl($scope, $state, $stateParams,$q,$verifyService,$timeout,$tourismService,$sce){
		...
		function getProductAct(){
			...
			$tourismService.getProductInfo({
				...
				vm.packageStatus = data.packageProduct.packageStatus[0];
				...
				if(data.packageProduct.isPackage == "false"){
					...
					if(vm.act.product[0].placeStart[0] == null){
						vm.placeStart = vm.act.product[0].placeStart;
					}else{
					...
					vm.getRecommend = vm.getRecommend.replace("<br>","")
					vm.getTrip = vm.act.product[0].trips == null ? vm.act.product[0].trips : vm.act.product[0].trips[0].trip;
					vm.exclude = vm.act.product[0].feeExclude;
					vm.include = vm.act.product[0].feeInclude;
		...
		function Modift(){
    		...
    		var mainScroll = scroll('.dialog-content .content');
		}
		//		
		function Mobye(){
			var verificationDialog = new dialog().alert({
...
/*--------------------选择套餐-------------------*/
setMealCtrl.$inject = ['$scope','$state','$stateParams', '$verifyService','$productService','$timeout','$tourismService','$window'];
function setMealCtrl($scope, $state, $stateParams,$verifyService,$productService,$timeout,$tourismService,$window){
				...
				if(vm.goDate != "请选择"){
					if(vm.stock < vm.totalNum){
						...
						$tourismService.checkUserCreditLine({
							...
							if(data.response.state[0].code == "3"){
								toolTip(data.response.state[0].resultMessage[0])
							}else if(data.response.state[0].code == "5"){
								toolTip(data.response.state[0].resultMessage[0])
							...
							}else if(data.response.state[0].code == "8"){
								toolTip(data.response.state[0].resultMessage[0])
							}else if(data.response.state[0].code == "7"){
								toolTip(data.response.state[0].resultMessage[0])
							}else{
