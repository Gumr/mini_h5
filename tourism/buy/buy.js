...
define(['angular', 'css!./buy.css', 'common/script/lib/swiper.min.js'], function(angular) {
    ...
    function orderController($scope, $state, $verifyService, $timeout, $tourismService, $stateParams, $window, $http) {
        ...
        $scope.data = {
            repayment: {},
            planRepayAmount: '', //首次还款
            planRepayPrincipal: '', //每月还款
            planRepayInterest: '',
            written: '第一期服务费包含手续费：',
            serverFee: ''
        }
        ...
        function stagesCount(val) {
            $tourismService.getRepayPlan({
                channelNo: sessionStorage.channelId,
                periods: val,
                loanAmount: vm.money
            }).success(function(data) {
                if (data.resultCode == '0000') {
                    $scope.data.planRepayAmount = data.result.repayPlanList[0].planRepayAmount;
                    $scope.data.planRepayPrincipal = data.result.repayPlanList[0].planRepayPrincipal;
                    $scope.data.serverFee = data.result.repayPlanList[0].planRepayFee;
                    data.result.repayPlanList[0].planRepayInterest = data.result.repayPlanList[0].planRepayInterest + data.result.repayPlanList[0].planRepayFee;
                    $scope.data.repayment = data.result.repayPlanList;
                }
            })
        }
                ...
                $tourismService.getValidateOrder({
                    ...
                    if (data.resultCode == "1000") {
                        $tourismService.checkUserCreditLine({
                            ...
                            } else if (data.response.state[0].code == "10") {
                                $state.go('login', {
                                    flog: 'tourism-order',
                                    id: $stateParams.id,
                                    stages: vm.stages
